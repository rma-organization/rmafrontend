...
// src/pages/ListRequestsComponent.jsx
import React, { useEffect, useState } from "react";
import {
...
import { listRequests, updateRequestStatus } from "../../../services/api/RequestServices";
...
const statusOptions = ["Requested", "Approved", "Rejected", "Delivered"];
...
const ListRequestsComponent = () => {
  ...
  const handleSave = async (rowId, userName, requestsId) => {
    try {
      await updateRequestStatus(rowId, status);

      const targetRole = "engineer";

      await sendNotification({
        receiverRole: targetRole,
        message: `Request ${rowId} updated to ${status}`,
        type: "request",
        read: false,
      ...
      const updatedRequests = requests.map((row) =>
              ...
              <TableBody>
                {requests.map((row) => (
                  <TableRow key={row.id}>
                    ...
                    <TableCell>{row.requestedByUser?.username || "N/A"}</TableCell>
                    <TableCell>
                      {editStatus === row.id ? (
                        <Button
                          variant="contained"
                          size="small"
                          onClick={() => handleSave(row.id, row.requestedByUser?.username, row.id)}
                        >
                          Save
                        </Button>
                      ) : (
                        <Button
                          variant="outlined"
                          size="small"
                          onClick={() => handleEditClick(row.id, row.status)}
                        >
                          Edit
                        </Button>
                      )}
